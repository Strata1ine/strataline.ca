...
export function focusLock(node) {
	let focusable = Array.from(node.querySelectorAll(interactable)).filter(
		(el) => el.getAttribute('aria-disabled') !== 'true' && !el.hasAttribute('inert'),
	);
	let previousFocus;
	let focusedIdx = 0;
	...
	return {
		update: (active) => {
			...
			if (active) {
				...
				document.querySelectorAll(interactable).forEach((e) => e.removeAttribute('inert'));
