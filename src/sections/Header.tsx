		...
		<Dialog>
			{(popover) => {
				...
				return (
					<>
						<Dialog.Trigger
							class={cn('z-2 size-10 cursor-pointer touch-manipulation', styles.burger)}
						...
						<Dialog.Content
							class={cn(
								...
								'border-accent fixed top-20 right-0 bottom-0 left-0 z-2 flex flex-col justify-center gap-7 bg-white p-9 sm:absolute sm:inset-auto sm:right-0 sm:rounded-xl sm:border-2',
							)}
						>
							<ul class="contents">
								<For
									...
									children={(item, index) => (
										...
										>
											<a
												class={cn(styles.link, 'font-serif text-3xl font-semibold')}
												href={`#${item.id}`}
												tabindex="0"
