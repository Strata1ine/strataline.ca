...
export async function onRequest(context: { locals: App.Locals }, next: () => Promise<Response>): Promise<Response> {
  ...
  for (let i = 0; i < context.locals.services.length; i++) {
    const service = context.locals.services[i];
    images.set(service.id, await optimizeImages(service.data.covers));
  }

  context.locals.serviceCovers = images;
