...
import type { Props as Hero } from "./Hero.astro";
import { slideshow } from "@/frontend/slideshow";

export default function HeroComponent(props: { meta: Hero }) {
  const [idx, setIdx] = createSignal(0);

  const ImageCarousel = () => (
    <For each={props.meta.content}>
      {(image, i) => <Image image={image} widths={[400, 650, 1300]} />}
    </For>
  );
    ...
    <div
      ...
      ref={(el) =>
        slideshow(el, () => ({
          idx: idx(),
          setIdx: setIdx,
          length: props.meta.content.length,
          speed: props.meta.speed,
        }))
      }
    >
      ...
      <div>
        <h1 class="heading-6xl font-bold">{props.meta.title}</h1>
        ...
        <p class="desc-base mt-3">{props.meta.desc}</p>
